...
@_$add_isid = ->
  return map ( token ) =>
    token.isxid = @is_xidentifier token.image
    return token

#-----------------------------------------------------------------------------------------------------------
@_$rewrite_whitespace = ->
  return map ( token ) =>
  ...
  return PS.$gliding_window 2, ( section ) =>
    ...
    if type_0 is 'nl'
      if type_1 is 'indentation'
        ...
        level = 0
    is_dedent = level? and level < prv_level
    if is_dedent
      start             = start_0
      stop              = start
      blank             = no
...
@$coffee_lex = ->
  pipeline = []
  pipeline.push @_$coffee_lex()
  pipeline.push @_$add_isid()
  pipeline.push @_$rewrite_whitespace()
  pipeline.push @_$collapse_newlines()
  pipeline.push @_$add_blank()
  pipeline.push @_$add_start_stop()
  pipeline.push @_$add_indentation()
  # pipeline.push @_$add_indent_dedent()
  pipeline.push @_$add_indent()
  pipeline.push @_$add_dedent()
  pipeline.push @_$add_eofdedent()
...
transpile = ( text, handler ) ->
  ...
  Ø XXX.$coffee_lex()
  # Ø XXX.$rewrite()
  Ø XXX.$exponentiation()
  Ø XXX.$xidentifiers()
  Ø XXX.$key_with_sigil()
  Ø XXX.$keystring()
  Ø XXX.$relabel_slash()
  Ø XXX.$slash_as_dot()
  Ø XXX.$number_with_underscores()
  Ø XXX.$transpile_to_cs_tokens()
  Ø XXX.$show()
  Ø XXX.$cs_tokens_as_text()
  Ø map ( text ) ->
    urge '\n' + text
    return text
  ###
