...
(function() {
  ...
  this._XXX_grammar = function(G, $) {

    /* to replace `@new`, above */
    var R, get_rule, rule_name, _ref;
    $ = LODASH.merge($ != null ? $ : {}, G.$);
    R = G != null ? G : {};
    R.$ = $;
    _ref = library['rules'];
    for (rule_name in _ref) {
      get_rule = _ref[rule_name];
      if (R[rule_name] == null) {
        R[rule_name] = TYPES.isa_function(get_rule) ? get_rule(R, $) : get_rule;
      }
    }
    return R;
  };
